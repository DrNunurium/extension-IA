...
async function rebuildGroupsIndex() {
    ...
    for (const [k, v] of Object.entries(allData)) {
        if (v && typeof v === 'object' && v.source_id)
            items.push(v);
    }
...
chromeApi?.runtime?.onMessage.addListener((msg, _sender, sendResp) => {
    if (msg?.type !== 'NAVIGATE_TO_SOURCE' || !msg.payload)
        return;
    const { pageUrl, sourceId } = msg.payload;
    (async () => {
        try {
            // open the page in a new tab and navigate to the element
            const tab = await new Promise((resolve) => chromeApi.tabs.create({ url: pageUrl, active: true }, resolve));
            const tabId = tab.id;
                            ...
                            func: (sid) => {
                                try {
                                    ...
                                    if (!el) {
                                        ...
                                        el = Array.from(document.querySelectorAll('*')).find(e => e.dataset && (e.dataset.sourceId === sid || e.dataset.messageId === sid));
                                    }
                                    if (el) {
