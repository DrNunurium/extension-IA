...
function __(key: string, fallback?: string) {
  try {
    if (
      ...
      (chrome as any).i18n &&
      typeof (chrome as any).i18n.getMessage === 'function'
    ) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      chromeRuntime.sendMessage({ type: 'GET_CHROME_ACTIVE_COLOR' }, (resp) => {
        ...
        clearTimeout(timer);
        try {
          if (resp && resp.color) resolve(String(resp.color));
          else resolve(null);
        } catch (_) {
          resolve(null);
        }
  ...
  const filtered = normalizedQuery
    ? sorted.filter((it) => {
        const haystack = normalizeForSearch(
          [it.title, it.original_text, it.summary].filter(Boolean).join(' '),
        );
  ...
  storage.get(null, (all) => {
    try {
      ...
      for (const [k, v] of Object.entries(all || {})) {
        ...
        let normalizedItem =
          normalizeUrlString(v.pageUrl ?? null) ?? normalizeUrlString(v.normalized_page ?? null);
        if (!normalizedItem)
...
function getHighlightTarget(el) {
  ...
  const hasVisibleArea =
    rect && rect.width > 0 && rect.height > 0 && Boolean(el.textContent?.trim());
  if (hasVisibleArea) return el;
...
function findSnippetRangeWithFallback(root, snippetText) {
  const normalizedSnippet = normalizeForSearch(snippetText);
  if (!normalizedSnippet) return { range: null, matched: null };
  const { normalized, mapping } = buildNormalizedMap(root);
  if (!normalized) return { range: null, matched: null };
  ...
  if (idx !== -1) {
    ...
    if (r) return { range: r, matched: normalizedSnippet };
  }
  ...
  for (const len of windowLens) {
    if (normalizedSnippet.length <= len) continue;
    ...
    for (let start = 0; start + len <= normalizedSnippet.length; start += step) {
      ...
      if (pos !== -1) {
        ...
        if (r) return { range: r, matched: sub };
      }
    }
  }
...
function applySnippetHighlight(el, snippetText) {
  ...
  try {
    ...
    if (!extracted.childNodes.length) return null;
...
function highlightElement(el, snippetText) {
  ...
  try {
    const container =
      el instanceof HTMLElement && typeof el.closest === 'function'
...
function findExistingSourceElement(sourceId) {
  ...
  for (const el of Array.from(all)) {
    const ds = el.dataset || {};
    if (ds.sourceId === sourceId) return el;
  }
  return null;
}
function ensureMarkerForItem(item) {
  ...
  if (!sourceId) return;
  if (findExistingSourceElement(sourceId)) return;
  ...
  if (normalizedFull.length > 200) addSegment(normalizedFull.slice(0, 200));
  if (normalizedFull.length > 140) addSegment(normalizedFull.slice(0, 140));
  if (normalizedFull.length > 80) addSegment(normalizedFull.slice(0, 80));
  const matches = (text) => {
    const norm = normalizeForSearch(text || '');
    if (!norm) return false;
    return segments.some((seg) => {
      if (!seg) return false;
      if (seg.length >= 24) return norm.includes(seg);
  ...
  const tryCandidates = (elements) => {
    for (const el of elements) {
      if (!el || isInteractive(el)) continue;
  ...
  if (tryCandidates(orderedCandidates)) return;
  ...
  if (tryCandidates(childCandidates)) return;
...
function ensureMarkersForItems(items) {
  ...
  for (const item of items) ensureMarkerForItem(item);
}
...
function injectInto(container) {
  if (DISABLE_SAVE_BUTTONS) return; // no-op when save buttons are disabled
  try {
    ...
    const candidates = Array.from(
      container.querySelectorAll('h1,h2,h3,h4,h5,h6,p,li,blockquote,div'),
    ).filter(
      (n) =>
...
function scan(root) {
  for (const s of MESSAGE_SELECTORS) {
    ...
    if (!nodes) continue;
    nodes.forEach((n) => {
      if (n instanceof HTMLElement) injectInto(n);
    });
  }
}
...
function handlePossiblePageChange(force = false) {
  try {
    ...
    if (!force && current === lastKnownPageKey) return;
