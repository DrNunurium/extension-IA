...
function renderMindMapGraph(data: OrganizerMindMapData, updatedAt?: string) {
  ...
  type LayoutNode = { id: string; titulo: string; descripcion: string; x: number; y: number; central?: boolean; source_ids?: string[] };
  ...
  nodes.forEach((node, index) => {
    ...
    const ln: LayoutNode = { id: node.id || `n_${index}`, titulo: node.titulo || `Nodo ${index + 1}`, descripcion: node.descripcion || '', x, y, source_ids: Array.isArray((node as any).source_ids) ? (node as any).source_ids.slice() : [] };
    layoutNodes.push(ln);
  ...
  const highlightNode = (id: string, description: string, title: string) => {
    ...
    if (info) {
      ...
      try {
        const curEl = canvas.querySelector(`[data-node-id="${id}"]`) as HTMLElement | null;
        const raw = curEl?.dataset?.sourceIds;
        const srcIds: string[] = raw ? JSON.parse(raw) : [];
          ...
          for (const sid of srcIds) {
            ...
            btn.addEventListener('click', () => {
              try {
                ...
                chrome.runtime.sendMessage({ type: 'NAVIGATE_TO_SOURCE', payload: { pageUrl: targetPage, sourceId: sid } }, (_resp) => { /* ignore response */ });
              } catch (e) { console.error('Failed to request navigate', e); }
            });
            ...
            try {
              chrome.storage.local.get([sid], (res: Record<string, any>) => {
                const item = res?.[sid];
                if (item) {
            ...
            } catch (_) { preview.textContent = 'No se pudo cargar la vista previa.'; }
  ...
  try { group.dataset.sourceIds = JSON.stringify(ln.source_ids || []); } catch (_) { group.dataset.sourceIds = '[]'; }
